...
with open('.\Map_AI\map (3).osm', "rb") as osm_fn:
    ...
    for currentWay in range(ways_num):
        ...
        for firstnode_local_index in range(nodes_num):
            ...
            for othernode_local_index in range(firstnode_local_index + 1, nodes_num):
                ...
                if (firstnode_id != othernode_id and connectivity_matrix[firstnode_index, othernode_index] == float(
                        'inf')):
...
def dijkstra(source, connectivity_matrix, p):
    ...
    while i > 0:
        ...
        for j in range(v):
            if s.get(j, False) == False and connectivity_matrix[source][u] != float('inf') and connectivity_matrix[u][
                j] != float('inf'):
...
while (True):
    ...
    while (True):
        DestinationNode = int(
            input("Enter the selected Destination Node from the map or -1 to select a new node or 0 to exit :"))
